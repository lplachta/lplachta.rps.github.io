/*$(() => game.RockPaperScissors = new RockPaperScissorsGame());
class RockPaperScissorsGame {
	constructor(choices) {
		this.choices = ["rock", "paper", "scissors"];
		this.scoringMatrix = [["draw", "player", "computer"],
						   ["computer", "draw", "player"],
						   ["player", "computer", "draw" ]];
		this.moveButtons = $(".moveButtons.div");
		this.scoreboard = {
			player: $("#playerScore"),
		...
		function getChoices(){
			this.playerMove = this.getChoices.indexOf(event.target.id);
			this.computerMove = Math.floor(Math.random()*3);
		}
		function getWinner() {
			this.getWinner = this.scoringMatrix[this.computerMove][this.playerMove];
		}
